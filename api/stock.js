  ...
  const code = stockCode;
    ...
    if (type === 'news') {
      ...
      if (!newsData.success || newsData.items.length === 0) {
        // 뉴스 없으면 Groq에게 아는 정보로 분석 요청
        const fallback = await callAI(`"${query}" 주식 최근 동향과 뉴스를 분석해줘. JSON 배열만 응답.
[{"title":"제목","summary":"1줄요약","sentiment":"positive/negative/neutral","impact":"주가영향","time":"최근"}]`);
        return res.json({ result: fallback });
      }
      // 2. Groq로 분석 (최신 10개만)
      const recentItems = newsData.items.slice(0, 10);
      const newsText = recentItems.map((n,i) =>
        `${i+1}. [${n.date}] ${n.title}`
      ).join('\n');
      const aiResult = await callAI(`"${query}" 주식 뉴스 호재/악재 분석. JSON 배열만 응답. 설명 없이 JSON만.
${newsText}
형식: [{"title":"제목","summary":"1줄요약","sentiment":"positive/negative/neutral","impact":"주가영향","time":"날짜"}]`);
      ...
      try {
        ...
        arr.forEach((item, i) => {
          if (recentItems[i]?.date) item.time = recentItems[i].date;
        });
    ...
    try {
      ...
      const gr = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: '안녕 한 줄로만 대답해' }],
          max_tokens: 50
        })
      });
      const gd = await gr.json();
      groqTest = { status: gr.status, response: gd.choices?.[0]?.message?.content || JSON.stringify(gd).slice(0, 200) };
