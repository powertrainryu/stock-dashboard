...
async function loadKRX() {
  ...
  for (const searchType of ['13', '12']) {
    ...
    try {
      ...
      // Uint8Array로 받아서 latin1로 디코딩 (euc-kr 바이트 보존)
      ...
      // style="mso-number-format..." 속성을 가진 td에 종목코드가 있음
      // 코드는 ASCII(숫자)라 latin1/euc-kr 구분 없이 파싱 가능
      // 종목명은 euc-kr 바이트를 Buffer로 변환
      const tdPattern = /<td[^>]*mso-number-format[^>]*>(\d{6})<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>/g;
      let m;
      while ((m = tdPattern.exec(raw)) !== null) {
        const stockCode = m[1];
        // 종목명: latin1 바이트 → Buffer → euc-kr 디코딩
