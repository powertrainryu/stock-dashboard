...
async function searchStocks(query) {
  ...
  // 2. 종목명 → Gemini가 코드 찾기 + 한투로 검증
  const geminiResults = await searchByGemini(q);
  if (geminiResults.length > 0) return geminiResults;

  // 3. Yahoo fallback (미국 주식)
  return await searchYahoo(q);
}

// ── Gemini로 종목 코드 검색 ──
async function searchByGemini(query) {
  const KEY = process.env.GEMINI_API_KEY;
  if (!KEY) return [];
  try {
    const prompt = `주식 종목 "${query}" 의 종목코드를 알려줘. JSON 배열만 응답. 설명 없이 JSON만. 형식: [{"code":"종목코드","name":"종목명","market":"KOSPI또는KOSDAQ또는US"}] 최대5개. 한국주식은 6자리 숫자코드(예:005930), 미국주식은 티커심볼(예:AAPL). 확실하지 않으면 빈 배열[].`;
    ...
    for (const item of items.slice(0, 5)) {
      if (item.market === 'US') {
        if (item.code) verified.push(item);
        continue;
      }
      // 한국 주식: 한투로 코드 검증 및 정확한 종목명 확인
      if (/^\d{6}$/.test(item.code) && token) {
        try {
          const r2 = await fetch(`https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/search-stock-info?PRDT_TYPE_CD=300&PDNO=${item.code}&PRDT_NAME=`, {
            headers: { 'Content-Type': 'application/json', 'authorization': `Bearer ${token}`, 'appkey': APP_KEY, 'appsecret': APP_SECRET, 'tr_id': 'CTPF1604R', 'custtype': 'P' }
          });
          const d2 = await r2.json();
          const o = d2.output;
          if (o && o.shtn_pdno) {
            verified.push({ code: o.shtn_pdno, name: o.prdt_abrv_name || o.prdt_name, market: o.rprs_mrkt_kor_name?.includes('KOSPI') ? 'KOSPI' : 'KOSDAQ' });
            continue;
          }
        } catch(e) {}
      }
      if (item.code && item.name) verified.push(item);
    }
    return verified;
  } catch(e) {
