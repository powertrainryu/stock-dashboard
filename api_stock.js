...
export default async function handler(req, res) {
  ...
  const code = stockCode;
    ...
    if (type === 'news') {
      ...
      const aiResult = await callAI(`다음 "${query}" 주식 뉴스들을 분석해줘. JSON 배열만 응답. 다른 텍스트 없이 JSON만.
뉴스:
${newsText}

형식:
[{"title":"뉴스제목 그대로","summary":"2줄 핵심요약","sentiment":"positive/negative/neutral","impact":"주가영향 한줄","time":"날짜 그대로"}]`);

      // 날짜는 네이버에서 가져온 실제 날짜로 강제 적용
      try {
